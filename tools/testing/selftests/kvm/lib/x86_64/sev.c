			  ...
			  uint8_t page_type)
{
	...
	sparsebit_for_each_set_range(protected_phy_pages, i, j) {
		...
		if (vm->type == KVM_X86_SNP_VM) {
			...
			ret = snp_launch_update_data(vm, gpa_base + offset,
		...
		ret = sev_launch_update_data(vm, gpa_base + offset,
					     (uint64_t)addr_gpa2hva(vm, gpa_base + offset),
					     size);
		if (ret)
			return ret;
...
int snp_vm_launch(struct kvm_vm *vm, uint64_t policy, uint8_t flags)
{
